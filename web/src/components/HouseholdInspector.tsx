...
type Props={household:Household|null;language?:LifeLanguage;residentNames?:Record<string,string>;onClose:()=>void;onMemberSelect?:(npcId:string)=>void;className?:string}
...
export function HouseholdInspector({household,language='zh',residentNames={},onClose,onMemberSelect,className=''}:Props){
 ...
 const resourceRooms=household&&!rooms.length?Array.from(new Set(resources.map(resource=>resource.room_id))).map(id=>({id,kind:id,resource_ids:[]})):rooms
