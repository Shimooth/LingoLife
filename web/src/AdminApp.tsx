...
export function AdminApp(){
 ...
 <section className="admin-panel"><div className="panel-head"><div><h2>内测用户</h2><p>名称为 onboarding-test 或 onboarding-test-* 的专用测试账号可重复重置，无需重新消耗邀请码；账号、密码、额度和审计记录会保留。</p></div><form onSubmit={e=>{e.preventDefault();void load(query)}}><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜索用户名"/><button>搜索</button></form></div><div className="table-wrap"><table><thead><tr><th>用户</th><th>最后活跃</th><th>今日额度</th><th>状态</th><th>操作</th></tr></thead><tbody>{users.map(user=><tr key={user.id}><td><b>{user.username}</b><small>#{user.id}</small></td><td>{date(user.last_active_at)}</td><td>{quota(user)}</td><td><span className={`tag ${user.disabled?'off':'on'}`}>{user.disabled?'已禁用':'正常'}</span></td><td className="controls"><button disabled={busy} onClick={()=>void update(user,{quota_delta:10})}>赠送 10 条</button><button disabled={busy} onClick={()=>void update(user,{disabled:!user.disabled})}>{user.disabled?'启用':'禁用'}</button>{isOnboardingTestUser(user)&&<button className="admin-danger-button" disabled={busy} onClick={()=>void resetOnboarding(user)}>重置存档 / 新手流程</button>}</td></tr>)}</tbody></table>{!users.length&&<p className="no-data">没有找到用户。</p>}</div></section>
