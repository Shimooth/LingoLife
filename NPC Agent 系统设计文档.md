...
### 10.2.1 对某个人的记忆会淡化（2026-09-19）

不再由玩家点击“记下这一刻”决定 NPC 记忆。事件结算自动形成候选，AI 结合各自人格、关系和经历判断是否值得留存；A 对 B 与 B 对 A 可以不同。

记忆分为整体关系印象、近期清楚的经历、久远但深刻的往事。普通小事逐渐完全遗忘，重要经历逐渐丢失细节，只留大意；压缩跟随世界推进及后台批次持续执行，离线回来补算。反复查看历史不自动刷新记忆强度。

事件档案、关系后果及未解决矛盾不随记忆删除；遗忘不是原谅，也不是清空事实。生成新对白不能从历史档案绕过遗忘机制，不能替模糊记忆补编精确对白。当前实现范围、时间尺度及预算见 [居民双向记忆](docs/NPC_PAIR_MEMORY.md)。
