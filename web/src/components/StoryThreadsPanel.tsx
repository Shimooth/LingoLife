...
function StoryCard({story,language,kind,onSelect}:{story:SelectableStory;language:LifeLanguage;kind:'moment'|'incident'|'thread'|'aftermath';onSelect?:Props['onSelect']}){
 ...
 const outcome=storyHasOutcome(story)?storyOutcomeCopy(story,language):null
 const participants=story.participants?.map(person=>person.name).filter(Boolean).join(' · ')
 const eyebrow=kind==='incident'?(language==='zh'?'此刻正在发生':'Happening now'):kind==='aftermath'?(language==='zh'?'这件事留下的变化':'What this moment changed'):kind==='moment'&&!['resolved_autonomously','resolved_with_management','closed'].includes(story.status)?(language==='zh'?'生活里的一个片段':'A moment from daily life'):statusCopy(story,language)
 return <article className={`story-thread is-${kind}`}><small>{eyebrow}</small><h3>{title}</h3><p>{summary}</p>{outcome&&<p><strong>{outcome.result}</strong><br/>{outcome.choice}</p>}{aftermath&&<blockquote>{aftermath}</blockquote>}{participants&&<span>{participants}</span>}{onSelect&&<button type="button" className="story-thread__open" onClick={()=>onSelect(story)} aria-label={language==='zh'?`查看${title}`:`Open ${title}`}><i aria-hidden>›</i></button>}</article>
}
