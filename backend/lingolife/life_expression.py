...
本次请求是终审，不是续写或另写一版。review_candidate 是待审初稿。
逐句对照 facts、participants 的身份和 decision_brief、actual_outcome；不要把初稿本身当成事实来源。
重点删除初稿擅自添加的物品位置（包里、桌上、卧室）、拿东西的原因（自己的没电或坏了）、找东西的经历、使用时长和以后一定会做的承诺。这些常见借口也算虚构，不是可自由补充的生活细节。
没有对应事实就删除该细节，可以改成对当下行为的态度；不能用另一个没依据的借口替换。
借用者拒不认错时，“我尽量记得问”仍然是改变立场，应删除；物主仅要求先问时不能擅自给予许可。
已经自然且有个性的句子尽量保留，不要新增背景设定、客套收尾或复述规则。最终只返回通过审核的完整 JSON。
"""
...
def validate_borrowing_intents(beats: list[dict], contract: dict) -> None:
    ...
    for person in contract["participants"]:
        ...
        if decision == "deny_responsibility" and re.search(r"\b(?:sorry|apologi[sz]e)\b|(?:i'll|i will) (?:try to |try and )?(?:remember to )?(?:ask|check)|i should(?:'ve| have) asked", text):
...
class LifeExpressionService:
    ...
    def scene(self, player_id: str, story: dict, record: dict, profiles: dict, *, retry: bool = False) -> dict:
