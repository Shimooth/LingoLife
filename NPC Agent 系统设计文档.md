...
创建界面采用稀疏、可选的「与其他成员的关系」，而不是要求玩家填写所有两两组合。默认空白；用户主动选择成员后，添加亲属类型或自定义关系背景（如老同学、死党、前同事）。同一关系切换到另一方仍可查看／编辑，亲属类型由服务端同时保存对应方向；删除或重抽草稿居民会移除指向它的草稿关系。自定义背景存为双方可用的 `personal_connection`，并不是直接设定心理数值或正式恋爱状态；亲属必须选结构化类型，不能仅靠自由文本承担恋爱资格校验。未添加关系的居民从正常同住者开始，后续由实际生活形成关系。
