...
import {selectConversationOpening} from './conversationOpening'
import {defaultAvatar} from './avatar'
import {CharacterStudio} from './components/CharacterStudio'
import {OnboardingFlow} from './components/OnboardingFlow'
import {CityPracticeGuide} from './components/CityPracticeGuide'
import {useCityPractice} from './life/useCityPractice'
...
import {lifeStoryDialogue} from './api'
...
import {residentPresence,presentInHome} from './life/spatialPresence'
...
import {conversationLocation} from './life/conversationLocation'
...
import {VisualQualityContext} from './three/rendering/VisualQualityContext'
import type {ActiveEvent,AgentMemory,AgentState,AnimationCue,City,CityResident,EventUpdate,Feedback,Household,LearningProgress,LifeStory,Message,Mood,NpcEntry,NpcProfile,OnboardingCompleteRequest,OnboardingState,Quota,ResidentWorldAction,Room,SocialInteraction,Stats,User} from './types'
import type {WorldLayoutDocument} from './worldLayout'
type Pending={message:string;key:string}
const initialProfile:NpcProfile={name:'艾玛',age:25,relationship:'朋友',personality:['善良','体贴','安静'],interests:['绘画','音乐','摄影'],likes:['城市速写','轻柔的爵士乐','安静的咖啡馆'],dislikes:['被催促','不问就借走东西'],quirks:['思考时会把画框摆正'],habits:['睡前读书','下班后泡茶'],boundaries:['借私人物品前先问一声','进入私人空间前先敲门'],occupation:'设计师',longTermGoal:'开一家独立的小型设计工作室。',householdRole:'mediator',chorePreferences:['cleaning','dishes'],privateSpacePreference:'high',familyIds:[],householdWithIds:[],avatar:defaultAvatar}
...
export default function App({user,initialQuota,onLogout}:{user:User;initialQuota:Quota;onLogout:()=>void}){
 ...
 const [booting,setBooting]=useState(true),[onboarding,setOnboarding]=useState<OnboardingState|null>(null),[onboardingSaving,setOnboardingSaving]=useState(false),[onboardingError,setOnboardingError]=useState('')
 ...
 const [npcs,setNpcs]=useState<NpcEntry[]>([]),[npcId,setNpcId]=useState(localStorage.getItem('lingolife.npc-id')||'emma'),[creating,setCreating]=useState(false),[npcLimit,setNpcLimit]=useState(8)
 ...
 const [worldLayout,setWorldLayout]=useState<WorldLayoutDocument>()
 ...
 const [householdFocus,setHouseholdFocus]=useState<{id:string;request:number}>()
 ...
 const [conversation,setConversation]=useState<Room['conversation']>(null)
 ...
 const practice=useCityPractice(Boolean(onboarding?.completed&&city),city?.world_version)
 ...
 const showOpening=(room:Room)=>{const opening=selectConversationOpening(room);if(!opening?.text)return;turnRef.current+=1;const key=++speechKeyRef.current;setLiveSpeech({key,speaker:'npc',text:opening.text,translation:opening.translation})}
 ...
 const loadNpc=async(id:string,entries?:NpcEntry[],authorOpening=false)=>{
  ...
  try{
   ...
   const room=await api.room(chosen.id,authorOpening)
   setConversation(room.conversation??null)
 ...
 const load=async()=>{
  setBooting(true)
  try{
   let onboardingData:OnboardingState|null=null
   try{onboardingData=await api.onboarding()}catch(cause){if(!(cause instanceof ApiError)||cause.status!==404)throw cause}
   if(onboardingData){
    setOnboarding(onboardingData);setNpcLimit(onboardingData.max_residents)
    if(!onboardingData.completed){setNpcs([]);setCity(null);setWorldSeed(null);setReady(false);return}
   }
   const [data,cityData,layoutData]=await Promise.all([api.npcs(),api.city(),api.worldLayout().catch(()=>null)])
   if(layoutData)setWorldLayout(layoutData.layout)
   setNpcLimit(data.limit||onboardingData?.max_residents||8);setNpcs(data.npcs);setCity(cityData);setWorldSeed(cityData)
   if(!onboardingData)setOnboarding({version:0,completed:true,min_residents:2,max_residents:data.limit||8,resident_count:data.npcs.length,user_created_count:data.npcs.length,remaining_slots:Math.max(0,(data.limit||8)-data.npcs.length)})
   const id=data.npcs.some(x=>x.id===npcId)?npcId:data.npcs[0]?.id||'emma'
   await loadNpc(id,data.npcs);void api.learningProfile().then(setLearning).catch(()=>undefined)
  }catch(cause){fail(cause,t('character.loadError'))}finally{setBooting(false)}
 }
 ...
 const completeOnboarding=async(setup:Omit<OnboardingCompleteRequest,'household_name'>)=>{
  setOnboardingSaving(true);setOnboardingError('')
  try{
   const result=await api.completeOnboarding({household_name:'Cloudview House',...setup}),[cityData,layoutData]=await Promise.all([result.city?Promise.resolve(result.city):api.city(),api.worldLayout().catch(()=>null)])
   if(layoutData)setWorldLayout(layoutData.layout)
   setNpcs(result.npcs);setNpcLimit(result.onboarding.max_residents);setCity(cityData);setWorldSeed(cityData);setView('city')
   const first=result.npcs.find(entry=>entry.id!=='emma')??result.npcs[0]
   if(first){setNpcId(first.id);setProfile(first.profile);void loadNpc(first.id,result.npcs)}
   setOnboarding(result.onboarding);void api.learningProfile().then(setLearning).catch(()=>undefined)
  }catch(cause){if(cause instanceof ApiError&&cause.code==='ONBOARDING_ALREADY_COMPLETED'){await load();return}const messages:Record<string,string>=zh?{ROSTER_REVIEW_REQUIRED:'旧存档需要管理员确认，已有居民与历史不会被删除。',NPC_LIMIT_REACHED:'住宅最多容纳 8 人，请减少新增居民。',NPC_NAME_TAKEN:'已有同名居民，请换一个名字。',ONBOARDING_SETUP_IN_PROGRESS:'已有一批居民正在入住，请保留当前设定后重试。',INTRO_NOT_ACKNOWLEDGED:'请先返回并完成玩法介绍。'}:{};setOnboardingError(cause instanceof ApiError?(messages[cause.code||'']||(zh?'入住未完成，请保留设定后重试。':cause.message)):(zh?'连接超时或中断，设定仍保留。请重试，已保存的居民不会重复创建。':'Connection interrupted. Retry with these settings; saved residents will not be duplicated.'))}finally{setOnboardingSaving(false)}
 }
 const acknowledgeOnboardingIntro=async()=>{
  const state=await api.acknowledgeOnboardingIntro(1)
  setOnboarding(state)
 }
 const saveProfile=async()=>{setSavingProfile(true);setProfileError('');const wasCreating=creating;try{const explicitProfile=normalizeNpcProfilePolicy({...profile,avatar:{...profile.avatar,strokes:[]}},creating?undefined:npcId);const saved=creating?await api.createNpc(explicitProfile):await api.saveNpc(npcId,explicitProfile);const data=await api.npcs();setNpcLimit(data.limit||8);setNpcs(data.npcs);await refreshWorld('mutation');setCreating(false);setStudio(false);await loadNpc(saved.id,data.npcs);if(wasCreating)setView('city')}catch(cause){setProfileError(cause instanceof ApiError&&cause.code==='NPC_LIMIT_REACHED'?t('character.limit'):t('character.saveError'))}finally{setSavingProfile(false)}}
 ...
 const cityCharacters:CityCharacter[]=(city?.npcs||[]).map(resident=>{
  const location=city?.locations.find(item=>item.id===resident.current_location_id)
  const resource=getLocationAsset(location?.id,location?.kind)
  const lifeAction=normalizeResidentAction(resident),worldAction=displayedWorldAction(resident,lifeAction)
  const storyContext=[...(city?.open_incidents||[]),...(city?.observable_moments||[]),...(city?.story_threads||[])].find(story=>story.participant_ids.includes(resident.id))
  const relatedNpcId=(lifeAction?.source==='life'?lifeAction.targetNpcId:undefined)??storyContext?.participant_ids.find(id=>id!==resident.id)
  const relationshipContext=relatedNpcId?city?.relationships?.find(item=>item.participant_ids.includes(resident.id)&&item.participant_ids.includes(relatedNpcId)):undefined
  return {
   id:resident.id,name:resident.name,avatar:resident.avatar,animationCue:worldAction.animation_cue??resident.animation_cue,
   spatialPresence:residentPresence(resident),homeLocationId:resident.home.id,
   worldAction,lifeAction,visibleIntent:resident.visible_intent,visibleIntentZh:resident.visible_intent_zh,
   troubleSignal:resident.trouble_signal,observableState:resident.observable_state??(lifeAction?.source==='life'?(lifeAction.raw.observable_state??lifeAction.presentation?.observable_state):undefined),storyContext,relationshipContext,
   householdId:resident.household_id,home:{x:resident.home.x*1200/(city?.map.width||1600),y:resident.home.y*760/(city?.map.height||1000)},
   locationId:resident.is_home?undefined:location?.id,
   location:{x:resident.position.x*1200/(city?.map.width||1600),y:resident.position.y*760/(city?.map.height||1000),place:resident.is_home?t('map.location.home'):(location?locationCopy(resource,language).name:t('map.currentLocation'))},
  }
 })
 ...
 const focusedResidents=(city?.npcs||[]).filter(item=>residentPresence(item).mode!=='traveling'&&item.current_location_id===focusedLocationId).map(item=>({id:item.id,name:item.name}))
 ...
 const openChat=async(id:string)=>{const room=await loadNpc(id,undefined,true);if(!room)return;setFollowedCharacterId(id);setSocialEventId(undefined);setChromeCollapsed(true);setView('chat');showOpening(room)}
 ...
 const followResident=(id:string|undefined)=>{setFollowedCharacterId(id);if(id){setFocusedLocationId(undefined);if(practice.data?.progress.status==='active'&&practice.data.progress.step==='discover')void practice.command({event:'follow',npc_id:id})}}
 const startPractice=async()=>{const result=await practice.command({event:'start'});if(result){setSettingsOpen(false);setView('city')}}
 const focusLifeStory=async(story:LifeStory)=>{if(practice.data?.progress.status==='active'&&practice.data.progress.step==='participate'&&story.participant_ids.length>=2&&story.level!=='thread'){const selected=await practice.command({event:'select_story',story_id:story.id});if(!selected)return}selectedLifeStoryMutationRef.current=story;setStoriesOpen(false);setSocialOpen(false);setSocialEventId(undefined);setSelectedLifeStory(story);setView('city');if(story.location_id){setFollowedCharacterId(undefined);setFocusedLocationId(story.location_id)}else if(story.participant_ids[0]){setFocusedLocationId(undefined);setFollowedCharacterId(story.participant_ids[0])}if(story.level==='moment')setDismissedMomentIds(ids=>[...ids.filter(id=>id!==story.id),story.id].slice(-30))}
 ...
 const openHousehold=async(id:string,npcId?:string)=>{const embedded=city?.households?.find(item=>item.id===id);setHouseholdFocus(previous=>npcId?{id:npcId,request:(previous?.request??0)+1}:undefined);setHouseholdId(id);setHouseholdDetail(embedded??null);if(embedded)return;try{setHouseholdDetail(await api.household(id))}catch(cause){setHouseholdId(undefined);fail(cause,zh?'暂时无法打开这所住宅。':'This household is unavailable right now.')}}
 const returnToCity=async()=>{resetSpeech();setHistoryOpen(false);const resident=city?.npcs.find(item=>item.id===npcId);if(resident&&residentPresence(resident).mode==='indoor')setFollowedCharacterId(undefined);setView('city');await refreshWorld('mutation')}
 ...
 const householdResidentVisuals=householdId?(city?.npcs||[]).filter(resident=>resident.household_id===householdId).map(resident=>{
  const action=normalizeResidentAction(resident)
  const occupiedRoom=resident.current_room_id??selectedHousehold?.resources?.find(resource=>resource.state.occupied_by?.includes(resident.id))?.room_id
  return {id:resident.id,name:resident.name,avatar:resident.avatar,isHome:presentInHome(resident),roomId:occupiedRoom,currentAction:action,animationCue:resident.animation_cue,observableState:resident.observable_state??(action?.source==='life'?(action.raw.observable_state??action.presentation?.observable_state):undefined)}
 }):[]
 const currentResident=city?.npcs.find(resident=>resident.id===npcId)
 const currentConversationId=currentResident?.conversation_id
 useEffect(()=>{
  if(view!=='chat'||!ready||busy||pending||!conversation?.id||!currentConversationId||conversation.id===currentConversationId)return
  let cancelled=false
  // Keep drafts and historical messages; refresh only the live encounter.
  setLiveSpeech(null)
  void api.room(npcId).then(room=>{
   if(cancelled)return
   setConversation(room.conversation??null)
   setMessages(room.messages);setArchivedCount(room.messages.length)
   setStats(room.stats);setMood(room.npc.animation);setAnimationCue(room.npc.animation_cue??room.npc.animation)
   setActiveEvent(room.active_event??null);setEventUpdate(null);setFeedback(null)
   const opening=selectConversationOpening(room)
   if(opening?.text){turnRef.current+=1;setLiveSpeech({key:++speechKeyRef.current,speaker:'npc',text:opening.text,translation:opening.translation})}
  }).catch(()=>{/* The next authoritative world refresh retries; keep the draft. */})
  return()=>{cancelled=true}
 },[view,ready,busy,pending,npcId,conversation?.id,currentConversationId,city?.server_time])

 const chatLocation=conversationLocation(currentResident,city?.locations)
 const currentLocationAsset=chatLocation.asset
 const currentPlace=chatLocation.isHome?t('map.home'):(chatLocation.id?locationCopy(currentLocationAsset,language).name:t('map.currentLocation'))
 ...
 const lifeStoryAsset=selectedLifeStory?(lifeStoryLocation?getLocationAsset(lifeStoryLocation.id,lifeStoryLocation.kind):getLocationAsset('home')):undefined
 const lifeStoryLocationName=selectedLifeStory?(lifeStoryLocation?locationCopy(lifeStoryAsset!,language).name:(selectedLifeStory.household_id?t('map.home'):selectedLifeStory.location_id)):undefined
 ...
 const overlays=<AnimatePresence>
  ...
  {settingsOpen&&<motion.div key="settings" className="experience-overlay-layer" initial={reduce?false:{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><SettingsPanel onPractice={()=>void startPractice()} practiceBusy={practice.busy} practiceFailed={practice.failed} hasPassword={hasPassword} onPasswordSet={()=>setHasPassword(true)} onClose={()=>setSettingsOpen(false)}/></motion.div>}
  {storiesOpen&&<StoryThreadsPanel key="life-stories" moments={city?.observable_moments||[]} threads={city?.story_threads||[]} incidents={city?.open_incidents||[]} recentAftermath={city?.recent_aftermath||[]} relationships={city?.relationships||[]} attentionBudget={city?.attention_budget} residentNames={residentNames} legacyCount={city?.social_interactions?.length||0} language={language} onClose={()=>setStoriesOpen(false)} onSelect={focusLifeStory} onOpenLegacy={()=>{setStoriesOpen(false);setSocialOpen(true)}}/>}
  ...
  {selectedLifeStory&&<LifeStoryEncounter key={`life-story:${selectedLifeStory.id}`} story={selectedLifeStory} onDialogue={lifeStoryDialogue} guideStep={practice.data?.progress.status==='active'&&practice.data.progress.story_id===selectedLifeStory.id?practice.data.progress.step:undefined} guideBusy={practice.busy} guideFailed={practice.failed} onGuideComplete={async()=>{const result=await practice.command({event:'acknowledge_result',story_id:selectedLifeStory.id});if(result?.progress.status==='completed'){setSelectedLifeStory(undefined);setStoriesOpen(true)}}} onGuideRefresh={()=>void practice.refresh()} locationName={lifeStoryLocationName} locationImage={lifeStoryAsset?.image} participantAvatars={participantAvatars} layoutRooms={worldLayout?.interior.rooms} language={language} onClose={()=>{selectedLifeStoryMutationRef.current=undefined;setSelectedLifeStory(undefined)}} onObserve={async story=>{const result=await api.observeLifeStory(story.id);selectedLifeStoryMutationRef.current=result;setSelectedLifeStory(current=>current?.id===result.id?result:current);void practice.refresh();await refreshWorld('mutation');return result}} onIntervene={async(story,action)=>{const result=await api.interveneLifeStory(story.id,action,api.key());selectedLifeStoryMutationRef.current=result;setSelectedLifeStory(current=>current?.id===result.id?result:current);void practice.refresh();await refreshWorld('mutation');return result}}/>}
  {householdId&&<HouseholdInspector key={householdId} residentFocus={householdFocus} household={selectedHousehold} gameDate={city?.date} language={language} residentNames={residentNames} residentVisuals={householdResidentVisuals} layoutRooms={worldLayout?.interior.rooms} onClose={()=>{setHouseholdId(undefined);setHouseholdDetail(null)}} onDinnerCommand={async action=>{await dinnerApi.command(householdId,action);await refreshWorld('mutation')}} onMemberSelect={id=>{setHouseholdId(undefined);setFocusedLocationId(undefined);const resident=city?.npcs.find(item=>item.id===id);if(resident&&residentPresence(resident).mode==='indoor'){void openChat(id);return}setFollowedCharacterId(id);setView('city')}}/>}
 ...
 const chatView=<motion.main key="chat" className="shell experience-view" initial={reduce?false:{opacity:0,scale:.985,x:18}} animate={{opacity:1,scale:1,x:0}} exit={reduce?{opacity:0}:{opacity:0,scale:.99,x:24}} transition={{duration:reduce?0:.34,ease:[.22,.8,.25,1]}}><section className={`room dialogue-room ${chromeCollapsed?'is-chrome-collapsed':''}`}>
  <header className="dialogue-header">
   <div className="dialogue-header__content" id="chat-topbar-content" aria-hidden={chromeCollapsed} inert={chromeCollapsed}><div><button className="back-city" onClick={()=>void returnToCity()}>← {t('map.back')}</button><p className="room-location">⌖ {currentPlace}</p></div><div className="account"><select className="npc-switch" value={npcId} disabled={busy} onChange={e=>void (async()=>{const room=await loadNpc(e.target.value,undefined,true);if(room)showOpening(room)})()} aria-label={t('nav.switchCharacter')}>{npcs.map(x=><option key={x.id} value={x.id}>{x.profile.name}</option>)}</select><button className="learn-button" onClick={()=>setLearningOpen(true)}>◒ {t('nav.progress')}</button><button onClick={()=>setSettingsOpen(true)}>⚙ {t('nav.settings')}</button><span><b>{user.username}</b><small>{t('nav.quota',{count:quota.remaining})}</small></span></div></div>
 ...
 if(booting)return <BrandedStartupShell message={zh?'正在准备你的城市…':'Preparing your city…'}/>
 if(onboarding&&!onboarding.completed)return <OnboardingFlow language={language} minimum={onboarding.min_residents} maximum={Math.min(onboarding.max_residents,onboarding.remaining_slots||onboarding.max_residents)} introAcknowledged={Boolean(onboarding.intro_acknowledged_at&&onboarding.intro_version===1)} saving={onboardingSaving} error={onboardingError} retainedResidents={onboarding.resident_count} onAcknowledgeIntro={acknowledgeOnboardingIntro} onComplete={setup=>void completeOnboarding(setup)}/>
 return <VisualQualityContext value={experience.quality}><AnimatePresence mode="wait" initial={false}>{view==='city'?cityView:chatView}</AnimatePresence>{view==='city'&&currentMoment&&!selectedLifeStory&&practice.data?.progress.status!=='active'&&<div className={`life-moment-layer ${chromeCollapsed?'is-collapsed':''} ${followedCharacterId?'has-follow-card':''}`}><MomentToast moment={currentMoment} language={language} onOpen={()=>focusLifeStory(currentMoment)} onDismiss={()=>setDismissedMomentIds(ids=>[...ids.filter(id=>id!==currentMoment.id),currentMoment.id].slice(-30))}/></div>}{view==='city'&&city&&!selectedLifeStory&&!selectedSocialEvent&&!householdId&&!storiesOpen&&!socialOpen&&!settingsOpen&&!studio&&!learningOpen&&<CityPracticeGuide data={practice.data} residents={city.npcs} language={language} busy={practice.busy} failed={practice.failed} onHome={()=>{const home=city.households?.[0];if(home)void openHousehold(home.id)}} onFollow={followResident} onOpen={focusLifeStory} onPause={()=>void practice.command({event:'pause'})} onResume={()=>void practice.command({event:'start'})} onRefresh={()=>void practice.refresh()}/>} {overlays}</VisualQualityContext>
}
