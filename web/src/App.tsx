...
import type {ActiveEvent,AgentMemory,AgentState,AnimationCue,City,CityResident,EventUpdate,Feedback,Household,LearningProgress,LifeStory,Message,Mood,NpcEntry,NpcProfile,OnboardingState,Quota,ResidentWorldAction,Room,SocialInteraction,Stats,User} from './types'
...
export default function App({user,initialQuota,onLogout}:{user:User;initialQuota:Quota;onLogout:()=>void}){
 ...
 const completeOnboarding=async(profiles:NpcProfile[])=>{
  setOnboardingSaving(true);setOnboardingError('')
  try{
   const result=await api.completeOnboarding({household_name:'Cloudview House',residents:profiles}),[cityData,layoutData]=await Promise.all([result.city?Promise.resolve(result.city):api.city(),api.worldLayout().catch(()=>null)])
   if(layoutData)setWorldLayout(layoutData.layout)
 ...
 if(onboarding&&!onboarding.completed)return <OnboardingFlow language={language} minimum={onboarding.min_residents} maximum={Math.min(onboarding.max_residents,onboarding.remaining_slots||onboarding.max_residents)} saving={onboardingSaving} error={onboardingError} onComplete={profiles=>void completeOnboarding(profiles)}/>
