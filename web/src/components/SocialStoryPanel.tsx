...
export function SocialStoryPanel({events,language,onClose,onFocus,onIntervene}:{events:SocialInteraction[];language:'zh'|'en';onClose:()=>void;onFocus:(event:SocialInteraction)=>void;onIntervene:(event:SocialInteraction,action:SocialAction)=>Promise<void>}){
 ...
 return <motion.aside className="social-story-panel" role="dialog" aria-modal="true" aria-label={zh?'岛上动态':'Island stories'} initial={reduce?{opacity:0}:{opacity:0,x:24,scale:.97}} animate={{opacity:1,x:0,scale:1}} exit={{opacity:0,x:15}}>
  <header><div><small>{zh?'观察者简报':'OBSERVER BRIEFING'}</small><h2>{zh?'岛上正在发生':'Happening on the island'}</h2></div><button type="button" onClick={onClose}>×</button></header>
  <div className="social-story-panel__list">{events.length?events.map(event=>{const story=zh?chineseStory(event):[event.title,event.summary];return <motion.article layout key={event.id} className={event.status==='awaiting_management'?'needs-manager':''}>
   ...
   {event.management.can_intervene&&<div className="social-story-panel__actions"><b>{zh?'需要管理者决定':'Manager decision'}</b>{event.management.actions.map(action=><button type="button" disabled={Boolean(busy)} key={action} onClick={()=>void act(event,action)}>{busy===`${event.id}:${action}`?'…':labels[language][action]}</button>)}</div>}
   {!event.management.can_intervene&&<span className="social-story-panel__resolved">{event.status==='resolved_autonomously'?(zh?'居民们自行推进了这段生活。':'The residents carried this moment forward themselves.'):(zh?'这段故事已经产生结果。':'This story has reached an outcome.')}</span>}
  </motion.article>}):<p className="social-story-panel__empty">{zh?'今天岛上还没有居民间的新动态。创建第二位角色后，关系网络会开始自然运转。':'There are no new resident stories yet. Create a second character and their social world will begin to move.'}</p>}</div>
