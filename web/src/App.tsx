...
import type {ActiveEvent,AgentMemory,AgentState,AnimationCue,City,CityResident,EventUpdate,Feedback,LearningProgress,Message,Mood,NpcEntry,NpcProfile,Quota,ResidentWorldAction,Room,SocialInteraction,Stats,User} from './types'
...
export default function App({user,initialQuota,onLogout}:{user:User;initialQuota:Quota;onLogout:()=>void}){
 ...
 const [city,setCity]=useState<City|null>(null),[view,setView]=useState<'city'|'chat'>('city'),[settingsOpen,setSettingsOpen]=useState(false),[socialOpen,setSocialOpen]=useState(false),[focusedLocationId,setFocusedLocationId]=useState<string>()
 const [followedCharacterId,setFollowedCharacterId]=useState<string>(),[socialEventId,setSocialEventId]=useState<string>()
 ...
 const turnRef=useRef(0),speechKeyRef=useRef(0),streamTextRef=useRef(''),worldRefreshRef=useRef<Promise<City>|null>(null)
 ...
 const cityCharacters:CityCharacter[]=(city?.npcs||[]).map(resident=>{const location=city?.locations.find(item=>item.id===resident.current_location_id),resource=getLocationAsset(location?.id,location?.kind);return {id:resident.id,name:resident.name,avatar:resident.avatar,animationCue:resident.animation_cue,worldAction:synchronizedWorldAction(resident),home:{x:resident.home.x*1200/(city?.map.width||1600),y:resident.home.y*760/(city?.map.height||1000)},locationId:resident.is_home?undefined:location?.id,location:{x:resident.position.x*1200/(city?.map.width||1600),y:resident.position.y*760/(city?.map.height||1000),place:resident.is_home?t('map.location.home'):(location?locationCopy(resource,language).name:t('map.currentLocation'))}}})
 ...
 const cityNeedsAttention=Boolean(city?.npcs.some(resident=>synchronizedWorldAction(resident).state!=='idle')||city?.social_interactions?.some(item=>item.status==='traveling'||item.status==='awaiting_observation'||item.status==='awaiting_management'))
 ...
 const overlays=<AnimatePresence>{studio&&<motion.div key="studio" className="experience-overlay-layer" initial={reduce?false:{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><CharacterStudio language={language} profile={profile} onChange={setProfile} onSave={()=>void saveProfile()} onClose={()=>{setStudio(false);if(creating){setCreating(false);const current=npcs.find(x=>x.id===npcId);if(current)setProfile(current.profile)}}} saving={savingProfile} error={profileError}/></motion.div>} {learningOpen&&<motion.div key="learning" className="experience-overlay-layer" initial={reduce?false:{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><LearningPanel progress={learning} onClose={()=>setLearningOpen(false)}/></motion.div>} {settingsOpen&&<motion.div key="settings" className="experience-overlay-layer" initial={reduce?false:{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><SettingsPanel hasPassword={hasPassword} onPasswordSet={()=>setHasPassword(true)} onClose={()=>setSettingsOpen(false)}/></motion.div>} {socialOpen&&<SocialStoryPanel key="social" events={city?.social_interactions||[]} language={language} onClose={()=>setSocialOpen(false)} onFocus={focusSocialEvent} onIntervene={async(event,action)=>{await api.interveneSocial(event.id,action);await refreshWorld()}}/>} {selectedSocialEvent&&<SocialEventEncounter key="social-event" event={selectedSocialEvent} locationName={socialEventLocation?locationCopy(getLocationAsset(socialEventLocation.id,socialEventLocation.kind),language).name:undefined} locationImage={socialEventLocation?getLocationAsset(socialEventLocation.id,socialEventLocation.kind).image:undefined} participantAvatars={participantAvatars} language={language} onClose={()=>setSocialEventId(undefined)} onObserve={async event=>{const result=await api.observeSocial(event.id);await refreshWorld();return result}} onIntervene={async(event,action)=>{const result=await api.interveneSocial(event.id,action);await refreshWorld();return result}}/>} {agentOpen&&<AgentPanel key="agent" name={profile.name} agent={agent} onClose={()=>setAgentOpen(false)} onDeleteMemory={memory=>void deleteMemory(memory)}/>}</AnimatePresence>
  ...
  <section className={`city-screen ${chromeCollapsed?'is-chrome-collapsed':''}`}>
   <header className="city-header">
    <div className="city-header__content" id="city-topbar-content" aria-hidden={chromeCollapsed} inert={chromeCollapsed}>
     ...
     <div className="city-actions"><button disabled={npcs.length>=5} onClick={beginCreate}>＋ {t('nav.newCharacter')}</button><button onClick={()=>setSocialOpen(true)}>✦ {zh?'城市动态':'City stories'}{cityNeedsAttention&&<i/>}</button><button onClick={()=>setLearningOpen(true)}>◒ {t('nav.progress')}</button><button onClick={()=>setSettingsOpen(true)}>⚙ {t('nav.settings')}</button><span><b>{user.username}</b><small>{t('nav.quota',{count:quota.remaining})}</small></span><button onClick={async()=>{try{await api.logout()}finally{session.clear();onLogout()}}}>{t('nav.logout')}</button></div>
    </div>
