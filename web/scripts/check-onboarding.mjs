...
const connectionsSource=await (await import('node:fs/promises')).readFile(new URL('../src/components/OnboardingRelationships.tsx',import.meta.url),'utf8')
if(!flowSource.includes('<OnboardingRelationships'))fail('the optional relationship editor is not mounted')
if(connectionsSource.includes('<p>'))fail('the relationship editor reintroduced redundant explanation text')
for(const field of ['FAMILY_ROLE_INVERSE','personal_connection','与其他成员的关系','选填','remove(other.key)'])if(!connectionsSource.includes(field))fail(`the optional relationship editor is missing ${field}`)
if(flowSource.includes('otherDrafts.map'))fail('all pairs are still expanded in the onboarding editor')
const emptySocial=(await import('../src/onboardingProfiles.ts')).buildOnboardingSocialContract(fullHouse,[],[])
if(emptySocial.family_bonds.length||emptySocial.shared_history_hooks.length)fail('empty optional relationships manufactured facts')
const customSocial=(await import('../src/onboardingProfiles.ts')).buildOnboardingSocialContract(fullHouse,[],[{id:'connection-one',participantKeys:[fullHouse[0].key,fullHouse[7].key],kind:'personal_connection',summary:'死党',tone:'neutral'}])
if(customSocial.shared_history_hooks.length!==1||customSocial.shared_history_hooks[0].participant_indices.join(',')!=='0,7'||customSocial.shared_history_hooks[0].summary!=='关系：死党')fail('a custom sparse relationship did not preserve its target or short Chinese label')
