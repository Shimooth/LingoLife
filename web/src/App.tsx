...
export default function App({user,initialQuota,onLogout}:{user:User;initialQuota:Quota;onLogout:()=>void}){
 ...
 const householdResidentVisuals=householdId?(city?.npcs||[]).filter(resident=>resident.household_id===householdId).map(resident=>{
  ...
  const occupiedRoom=selectedHousehold?.resources?.find(resource=>resource.state.occupied_by?.includes(resident.id))?.room_id
  return {id:resident.id,name:resident.name,avatar:resident.avatar,isHome:resident.is_home,roomId:occupiedRoom,currentAction:action,animationCue:resident.animation_cue,observableState:resident.observable_state??(action?.source==='life'?(action.raw.observable_state??action.presentation?.observable_state):undefined)}
 }):[]
