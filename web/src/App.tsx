...
const initialProfile:NpcProfile={name:'Emma',age:25,relationship:'Friend',personality:['kind','thoughtful','quiet'],interests:['art','music','photography'],likes:['urban sketching','soft jazz','quiet cafés'],dislikes:['being rushed','borrowing without asking'],quirks:['straightens picture frames when thinking'],habits:['read before sleep','make tea after work'],boundaries:['ask before borrowing personal things','knock before entering private space'],occupation:'Designer',longTermGoal:'Open a small independent design studio.',householdRole:'mediator',chorePreferences:['cleaning','dishes'],privateSpacePreference:'high',familyIds:[],householdWithIds:[],avatar:defaultAvatar}
...
export default function App({user,initialQuota,onLogout}:{user:User;initialQuota:Quota;onLogout:()=>void}){
 ...
 const completeOnboarding=async(setup:Omit<OnboardingCompleteRequest,'household_name'>)=>{
  ...
  }catch(cause){setOnboardingError(cause instanceof ApiError?cause.message:(zh?'无法安置居民，请检查网络后重试。':'Could not move the residents in. Check your connection and try again.'))}finally{setOnboardingSaving(false)}
 }
