...
assert.match(studioSource,/familyRelations\.find\(item=>item\.targetId===id\)/,'the character editor must render typed family facts')
assert.doesNotMatch(studioSource,/set\(["']familyIds["']/,'the single-resident editor must not create a one-sided family fact')
assert.match(studioSource,/shared_history_hooks/,'the character editor must persist editable shared-history hooks')
