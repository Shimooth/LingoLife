...
const lifeAction={id:'life-read',type:'read',status:'performing',location_id:'home-emma-study',started_at:'2026-08-28T10:00:00Z',ends_at:'2026-08-28T10:02:00Z'}
...
assert.match(appSource,/paused=\{Boolean\(selectedLifeStory\)\}/,'the city renderer must pause behind a life-story encounter')
...
assert.match(studioSource,/set\("householdWithIds",event\.target\.value\?\[event\.target\.value\]:\[\]\)/,'living arrangements must stay single-select')
...
console.log('Life simulation frontend guard passed (13 actions, living presentation, romance consent, v2 precedence, version safety, single-timer schedule).')
