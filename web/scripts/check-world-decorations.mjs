...
if(!observerSource.includes("get('mapEditor')==='1'"))fail('author tools are not protected by the query-string gate')
if(!observerSource.includes('active:mapEditorAvailable&&decorationEditor.enabled'))fail('author interactions are no longer query-gated')
if(observerSource.includes('decorationEditor={mapEditorAvailable?'))fail('saved local decorations disappear outside author mode')
...
console.log(`World decoration guard passed (snap, exclusion zones, audit, import, author gate; legal sample ${legal.join(',')}).`)
